...
def main(argv: list[str] | None = None) -> int:
    ...
    processor = FrameworkProcessor(
        ...
        env={
            # The container installs requirements.txt from source_dir, then the
            # entrypoint installs the AWS extras. Region and bucket travel as
            # environment variables so configs/aws.yaml does not have to be
    ...
    print(f"Gửi Processing Job {job_name} ({instance_type})...", flush=True)
    processor.run(
        code=ENTRYPOINT,
        source_dir=str(REPOSITORY_ROOT),
        arguments=job_arguments,
